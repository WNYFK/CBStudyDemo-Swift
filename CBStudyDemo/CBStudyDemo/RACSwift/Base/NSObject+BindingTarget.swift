...
extension Reactive where Base: AnyObject {
	...
	public func makeBindingTarget<U>(on scheduler: Scheduler = UIScheduler(), _ action: @escaping (Base, U) -> Void) -> BindingTarget<U> {
		return BindingTarget(on: scheduler, lifetime: moviePro.lifetime(of: base)) { [weak base = self.base] value in
			if let base = base {
				action(base, value)
			}
		}
...
extension Reactive where Base: AnyObject {
	...
	public subscript<Value>(keyPath: ReferenceWritableKeyPath<Base, Value>) -> BindingTarget<Value> {
		return BindingTarget(on: UIScheduler(), lifetime: moviePro.lifetime(of: base), object: base, keyPath: keyPath)
	}
	...
	public subscript<Value>(keyPath: ReferenceWritableKeyPath<Base, Value>, on scheduler: Scheduler) -> BindingTarget<Value> {
		return BindingTarget(on: scheduler, lifetime: moviePro.lifetime(of: base), object: base, keyPath: keyPath)
	}
}
#endif
